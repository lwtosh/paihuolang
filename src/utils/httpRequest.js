...
http.interceptors.response.use(response => {
  if (response.data && (response.data.code === 'B19' && response.data.code === 'UNDEFINED')) { // 401, token失效
    clearLoginInfo()
    // window.location.reload()
    // var url = location.href
    // window.location.href = 'http://api.paihuo11.cn/to/toAuth?type=redirect&fromUrl=' + url
