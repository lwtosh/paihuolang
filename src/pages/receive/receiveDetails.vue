<template>
  ...
  <!-- 标题 -->
  <div class="pai">
      <div class="price"><span class="yen"> ￥</span>{{dataList.money}} <span style="font-size:.3rem">福利价</span></div>
      <div class="text-index">
        <div class="">
          市场价: <span style="text-decoration: line-through">￥{{dataList.goodsVo.marketPrice}}</span>
        </div>
        <div class="count">已领取 : {{dataList.payNum}}件</div>
      </div>
    </div>
  <div class="text">
  ...
  <div class="img">
    <img :src="item" :alt="item" v-for="(item,index) in imgUrl2" :key="index">
     ...
     <div class="btn" @click="payMent">免费领取</div>
<van-popup
...
export default {
  ...
  methods: {
    ...
    list () {
      this.$http({
        url: this.$http.adornUrl('/h5/welfare/fetchWelfareGoodsDetail'),
        method: 'get',
    ...
    payMent () {
      if (Vue.cookie.get('userId') === undefined) {
        ...
        this.$router.push({path: '/convert', query: {id: this.dataList.id, postage: this.dataList.postage, goodsName: this.dataList.goodsVo.goodsName, goodsCoverImg: this.dataList.goodsVo.goodsCoverImg, money: this.dataList.money, curUrl: location.href, type: this.dataList.type}})
      }
    },
    handleopen () {
    ...
    onClickWhart () {
      ...
      var obj = {
        ...
        linkurl: 'http://h5.paihuo11.cn/phlShop/index.html#/receiveDetails?id=' + this.$route.query.id + '&' + 'inviteCode' + '=' + Vue.cookie.get('inviteCode') ? Vue.cookie.get('inviteCode') : 'system', // 分享链接
    ...
    onClickFir () {
      ...
      var obj = {
        title: this.dataList.goodsVo.goodsName, // 分享标题
        desc: '直供货源、品质保证、拍的多、赚的多、买的多、返的多',
        linkurl: 'http://h5.paihuo11.cn/phlShop/index.html#/receiveDetails?id=' + this.$route.query.id, // 分享链接
        img: this.dataList.goodsVo.goodsCoverImg// 分享内容显示的图片
      }
      sdk.getJSSDK(url, obj)
    },
